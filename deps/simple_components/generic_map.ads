...
--
--  This  package  defines  the  type Map. An instance of the type is an
--  associative  array  of items. One can add and remove items to a map.
--  Items  of  the  map can be accessed using either a key or a positive
--  index. They are ordered, so a  map  implementation  can  use  binary
--  search.  Reference counting is used for the objects of the type Map,
--  which  means  that  assigning  Map  objects is relatively cheap. The
--  package is generic. It has the following generic parameters:
--
...
--     =            - Relational operator on keys
--
--  Both Key_Type and Object_Type can be controlled. The  implementation
--  warranties that when an item or key is no more used in the map it is
--  erased by assigning it a value created by  the  default  constructor
--  (if any). This behavior ensures that items and keys removed from the
--  map will be always  finalized  upon  the  operation.  The  parameter
--  Increment controls set size growth. When there is no free  space  in
--  the set then it is enlarged by Size * Increment / 100. Here Size  is
--  the current set size. The allocated amount of items cannot  be  less
--  than the parameter Minimal_Size specifies. So it will be the initial
--  set size after the first element is put in.
--
...
package Generic_Map is
   pragma Preelaborate (Generic_Map);
