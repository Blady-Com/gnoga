...
procedure Forms is
   ...
   begin
      ...
      App.Input.Create_Element (Form => App.My_Form, Input_Type => "text", Name => "Some_Text");

      Label.Create (Form => App.My_Form, Label_For => App.Input, Content => "Type Text:");

      Button1.Create (Form => App.My_Form, Value => "onclick button");
      Button1.On_Click_Handler (On_Click'Unrestricted_Access);

      Button2.Create (Form => App.My_Form, Value => "send to demo via post");

      Button3.Create (Form => App.My_Form, Value => "send to demo via get");
      Button3.On_Click_Handler (On_Submit_Get'Unrestricted_Access);

      App.Pick.Create (Form => App.My_Form, Name => "My_Color");
      ...
      begin
         Form2.Create (Parent => App.Console, Action => "/demo", Method => Form.Post);
         Form2.Encoding (Form.Multi_Part);
         Form2.Put ("File to upload: ");
         F_Input.Create_Element (Form => Form2, Input_Type => "file", Name => "fspec");
         H_Input.Create (Form => Form2, Value => "test", Name => "Some_Text");
         E_Input.Create_Element (Form => Form2, Input_Type => "text", Name => "Some_Text2");
   ...
   begin
      ...
      if Main_Window.Form_Parameter ("Some_Text") /= "undefined" then
         Last_Parameters.Include ("Some_Text", Main_Window.Form_Parameter ("Some_Text"));
      end if;

      --  If a post was stored in On_Post
      if Last_Parameters.Contains ("Some_Text") then
         Console.Put_Line ("Some_Text   => " & Last_Parameters.Element ("Some_Text"));
      else
      ...
      if Last_Parameters.Contains ("Some_Text2") then
         Console.Put_Line ("Some_Text2  => " & Last_Parameters.Element ("Some_Text2"));
      else
         Console.Put_Line ("Some_Text2  => No value sent");
      end if;

      if Last_Parameters.Contains ("file_name") then
         Console.Put_Line ("File Name on Server  => " & Last_Parameters.Element ("file_name"));
         Console.Put_Line ("File Name => " & Last_Parameters.Element ("temp_name"));
      end if;
   ...
   begin
      for C in Parameters.Iterate loop
         ...
         Gnoga.Log (Gnoga.Types.Data_Maps.Key (C) & " = " & Gnoga.Types.Data_Maps.Element (C));
      end loop;
   end On_Post;
