...
--
   ...
   procedure Allocate
      ...
      Upper : Storage_Count;
      Moved : Reference;
      Lower : Storage_Count := Get_Offset (Data, Data.Lower);
   begin
      ...
      if Upper + Storage.Length > Data.Size then
         ...
         if Lower >= Upper then
            ...
            -- |<Lower                                |
            --
            Moved := Data.Lower + Reference (Data.Size - Lower);
      ...
      then
         ...
         begin
            ...
            if Size > 0 then
               loop
                  ...
                  if Lower - Upper >= Storage.Length then
                     ...
                     --
                     Moved := Data.Lower + Reference (Lower - Old);
                     if Moved > Storage.Preserved then
                        raise Constraint_Error;
                     else
                        Data.Lower := Moved;
            ...
            --
            declare
               Old_Lower : constant Reference := Data.Lower;
            begin
               Moved :=
                  (  Old_Lower
                  +  Reference
...
<<Done>>
   ...
   begin
      return Storage.Data.Lower;
   exception
      when Constraint_Error =>
         if Storage.Data = null then
            Raise_Exception
            (  Status_Error'Identity,
               "Blackboard is not initialized"
            );
         else
            raise;
         end if;
   ...
   begin
      ...
      declare
         ...
         Size   : Storage_Offset;
      begin
         if Index >= Data.Upper then
            raise Constraint_Error;
         end if;
         Size := Get_Block_Size (Data, Offset) -
                 Aligned_Storage_Offset_Size;
         if Index < Data.Lower then -- If Size already overwritten
            raise Constraint_Error;
         end if;
         if Size <= 0 then
            if Size = -Aligned_Storage_Offset_Size then -- Wrapping
               Index := Index + Reference (Data.Size - Offset);
               if Index >= Data.Upper then -- Wrapped reference is
                  raise Constraint_Error;  -- outside
               end if;
               Offset := 0;
               Size   := Get_Block_Size (Data, Offset) -
                         Aligned_Storage_Offset_Size;
               if Size <= 0 or else Index < Data.Lower then
                  raise Constraint_Error;
               end if;
            else
               raise Constraint_Error;
   ...
   begin
      declare
         Data : Blackboard_Data renames Storage.Data.all;
      begin
         return Pointer >= Data.Lower and then Pointer <  Data.Upper;
      end;
   exception
      when Constraint_Error =>
         if Storage.Data = null then
   ...
   begin
      ...
      begin
         ...
         if Size <= 0 then
            ...
            if Index >= Data.Upper then -- Wrapped to the end
               return;
            end if;
            Offset := 0;
            Size   := Get_Block_Size (Data, Offset);
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            Data.Upper := Pointer + Reference (Pool.Length);
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            -- accessible
            --
            Data.Upper := Pool.Pointer + Reference (Pool.Length);
            Release (Storage.Lock.all);
         exception
            when others =>
               Release (Storage.Lock.all);
   ...
   function "<" (Storage : Blackboard; Pointer : Reference)
      return Boolean is
   begin
      return LE (Storage.Data.Upper, Pointer);
   exception
      when Constraint_Error =>
         if Storage.Data = null then
            Raise_Exception
   ...
   function "<" (Pointer : Reference; Storage : Blackboard)
      return Boolean is
   begin
      return LT (Pointer, Storage.Data.Lower);
   exception
      when Constraint_Error =>
         if Storage.Data = null then
            Raise_Exception
   ...
   function ">" (Storage : Blackboard; Pointer : Reference)
      return Boolean is
   begin
      return GT (Storage.Data.Lower, Pointer);
   exception
      when Constraint_Error =>
         if Storage.Data = null then
            Raise_Exception
   ...
   function ">" (Pointer : Reference; Storage : Blackboard)
      return Boolean is
   begin
      return GE (Pointer, Storage.Data.Upper);
   exception
      when Constraint_Error =>
         if Storage.Data = null then
            Raise_Exception
