...
package body Password_Gen.UI is
   ...
   procedure On_Length_Change (Object : in out Gnoga.Gui.Base.Base_Type'Class);
   procedure On_Generate (Object : in out Gnoga.Gui.Base.Base_Type'Class);
   ...
   when E : others =>
      Gnoga.Log (Message => "On_Length_Change: " &
   ...
   procedure On_Generate (Object : in out Gnoga.Gui.Base.Base_Type'Class) is
      ...
      function Digest (Source : String) return String is
         --  Empty
      begin -- Digest
         return PragmARC.Unbounded_Integers.Image
            (PragmARC.Unbounded_Integers.Value
               ("16#" & GNAT.SHA512.Digest (Source) & '#'), Base => 36);
      end Digest;

      App : constant App_Ptr := App_Ptr (Object.Connection_Data);

      Domain : constant String :=
         Ada.Characters.Handling.To_Lower (App.Domain.Value);

      Salted_Input : constant String :=
          ':' & Domain & ':' & App.Master.Value & ':';

      Digit_1_Index : Natural;
      Digit_1       : Natural := 0;
      Digit_1_Rem   : Natural;
      Length        : Positive := App.Length.Value;
         --  The requested password length, 8 .. 16
      Letter_Count  : Natural  := 0;
      Has_Lower     : Boolean  := False;
      Has_Upper     : Boolean  := False;
      Symbol        : String (1 .. 1);
      Result        : String :=
         Digest (Salted_Input & Integer'Image (Length) & ':');
   begin -- On_Generate
      ...
      --  Length is now the number of characters from Result to include in the
      --  password, 7 .. 16
      ...
      App.Result.Value (Value => Result (1 .. Integer'Min (Digit_1, Length)) &
         Symbol & Result (Digit_1 + 1 .. Length));
   exception -- On_Generate
   when E : others =>
      Gnoga.Log (Message => "On_Generate: " &
         Ada.Exceptions.Exception_Information (E));
   end On_Generate;
   ...
   when E : others =>
      Gnoga.Log
         (Message => "On_Quit: " & Ada.Exceptions.Exception_Information (E));
   end On_Quit;
   ...
   begin -- On_Connect
      ...
      App.Domain.Required;
      App.Domain.Place_Holder
         (Value => "example.com, example.fr, example.co.uk");
      App.Domain_Label.Create
         (Form => App.Input, Label_For => App.Domain, Content => "Domain:");
      App.Input.New_Line;
      App.Master.Create (Form => App.Input);
      App.Master.Required;
      App.Master_Label.Create (Form      => App.Input,
                               Label_For => App.Master,
      ...
      App.Length_Label.Create (Form      => App.Input,
                               Label_For => App.Length,
                               Content  => "Password Length:");
      App.Length_Img.Create
        (Form => App.Input, Size => 2, Value => App.Length.Value);
      App.Input.New_Line;
      App.Symbol.Create (Form => App.Input);
      App.Symbol_Label.Create
         (Form => App.Input, Label_For => App.Symbol, Content => "Symbol:");
      App.Symbol.Add_Option (Value => No_Symbol, Text => No_Symbol);
      App.Symbol.Add_Option
         (Value => Auto_Symbol, Text => Auto_Symbol, Selected => True);

      All_Symbols : for I in Symbol_Set'Range loop
         App.Symbol.Add_Option
            (Value => (1 => Symbol_Set (I)), Text => (1 => Symbol_Set (I)));
