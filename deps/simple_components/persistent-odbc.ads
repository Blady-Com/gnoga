...
with GNU.DB.SQLCLI.API;              use GNU.DB.SQLCLI.API;
...
with GNU.DB.SQLCLI.API.Keys.Sets;    use GNU.DB.SQLCLI.API.Keys.Sets;
...
pragma Elaborate_All (GNU.DB.SQLCLI.API);

package Persistent.ODBC is
   pragma Elaborate_Body (Persistent.ODBC);
--
...
-- Returns :
--
--    True if the driver natively supports Unicode
--
-- Exceptions :
--
--    Constraint_Error - Not a valid handle to an ODBC storage
--
   function Unicode (Storage : Storage_Handle)
      return Boolean;
