...
package body Synchronization.Interprocess.Events is
   ...
   begin
      if Code = ERROR_INVALID_HANDLE then
         ...
         (  Status_Error'Identity,
            Message & "Event is not initialized or already finalized"
         );
      else
   ...
   begin
      if Owner then
         Object.External := CreateEvent (ManualReset => 1);
   ...
   begin
      if 0 = ResetEvent (Object.External) then
         Check ("Reset event fault: ");
      end if;
   end Reset;

   procedure Signal (Object : in out Event) is
   begin
      if 0 = SetEvent (Object.External) then
         Check ("Set event fault: ");
      end if;
   end Signal;
