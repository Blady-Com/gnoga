...
package body Othello_Pkg.Callbacks is
   ...
   begin
      New_Game(Othello       => App_Data.Othello,
               Playing_Board => App_Data.Playing_Board,
               Whose_Move    => App_Data.Whose_Move,
   ...
   procedure Make_Move (Object : in out Gnoga.Gui.Base.Base_Type'Class) is
      Button : Othello_Types.Cell_Button renames
                 Othello_Types.Cell_Button(Object);
   ...
   begin
      ...
      App.Othello.Table1.Create
         (Parent      => App.Othello.Vbox1,
          Layout      =>
             (Othello_Types.Valid_Row'Range =>
                    (Othello_Types.Valid_Column'Range =>
      ...
      for I in Othello_Types.Valid_Row loop
         for J in Othello_Types.Valid_Column loop
            ...
            App.Playing_Board(I,J).Button.Create
               (App.Othello.Table1.Panel(I,J).all);

            -- Set button layout
            App.Playing_Board(I,J).Button.Width(50);
            App.Playing_Board(I,J).Button.Height(50);
            App.Playing_Board(I,J).Button.Border;
            App.Playing_Board(I,J).Button.Background_Color
               (Gnoga.Types.Colors.Light_Gray);

            -- Initialze circles and set their layout
            App.Playing_Board(I,J).Pixmap.Create
               (App.Playing_Board(I,J).Button);
            ...
            App.Playing_Board(I,J).Pixmap.Top
               (App.Playing_Board(I,J).Button.Height/2
                - App.Playing_Board(I,J).Pixmap.Height/2);
            App.Playing_Board(I,J).Pixmap.Left
               (App.Playing_Board(I,J).Button.Width/2
                - App.Playing_Board(I,J).Pixmap.Width/2);
      ...
      -- Start a new game!
      New_Game(Othello       => App.Othello,
               Playing_Board => App.Playing_Board,
               Whose_Move    => App.Whose_Move,
