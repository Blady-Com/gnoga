...
private
   ...
   type Mutex is new Abstract_Shared_Object with record
      External : aliased HANDLE := INVALID_HANDLE_VALUE; -- A handle to
      Owner    : Task_ID        := Null_Task_Id; -- The mutex owner
      Count    : Natural        := 0; -- Number of times it was taken
      Timeout  : DWORD          := 10 * 1000;
      pragma Atomic (Owner);
