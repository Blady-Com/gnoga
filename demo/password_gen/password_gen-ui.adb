...
package body Password_Gen.UI is
   ...
   begin -- On_Generate
      ...
      -- Length >= 7, so if not Has_Lower, then Result (1 .. Length) has at most 1 letter, which was capitalized by Lower_Some
      -- if not Has_Upper, then Result (1 .. Length) has at most 1 letter, which was not capitalized by Lower_Some
